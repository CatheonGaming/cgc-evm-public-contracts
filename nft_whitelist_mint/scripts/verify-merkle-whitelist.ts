...
try {
  ...
  Object.keys(json.list).forEach((address) => {
    const claim = json.list[address];
    ...
    if (verifyProof(claim.index, address, claim.amount, proof, merkleRoot)) {
      ...
      console.error("Verification for", address, "failed");
      valid = false;
    }
  });
  ...
  console.error(`Verify merkle whitelist failed: ${e.message}`);
}
