...
import { END_TIME1, MERKLE_ROOT1, MINT_PRICE1, PUBLIC_MINT_PRICE, START_TIME1 } from "./params";
...
task("set:WhitelistSale", "Set whitelist sale to WhitelistMintNFT nft contract")
  ...
  .setAction(async function (taskArguments: TaskArguments, hre) {
    ...
    // Deploy Contract
    const tx = await whitelistMintNft.setWhitelistSale(MERKLE_ROOT1, START_TIME1, END_TIME1, MINT_PRICE1);
    await tx.wait();

    console.log("New whitelist sale was set by transaction:", tx.hash);
  });

task("set:EnablePublicSale", "Set enable public sale to WhitelistMintNFT contract")
  .addParam("address", "The deployed smart contract address")
  .setAction(async function (taskArguments: TaskArguments, hre) {
    const whitelistMintNft = await hre.ethers.getContractAt("WhitelistMintNFT", taskArguments.address);
...
task("set:DisablePublicSale", "Set disable public sale to WhitelistMintNFT contract")
  .addParam("address", "The deployed smart contract address")
  .setAction(async function (taskArguments: TaskArguments, hre) {
    const whitelistMintNft = await hre.ethers.getContractAt("WhitelistMintNFT", taskArguments.address);
...
task("set:PublicMintPrice", "Set whitelist sale to WhitelistMintNFT contract")
  .addParam("address", "The deployed smart contract address")
  .setAction(async function (taskArguments: TaskArguments, hre) {
    const whitelistMintNft = await hre.ethers.getContractAt("WhitelistMintNFT", taskArguments.address);

    // Deploy Contract
    const tx = await whitelistMintNft.setPublicMintPrice(PUBLIC_MINT_PRICE);
    await tx.wait();
